...
#pragma warning restore CS0618
    ...
    class SheetRadiusDrawable: GradientDrawable
    {
    ...
    MaterialBottomSheet _sheet { get; }
    ...
    internal void UpdateBackground()
    {
        ...
        if (Frame is not null)
        {
            ...
            {
                var radiusDrawable = Frame.Background as SheetRadiusDrawable ?? new SheetRadiusDrawable();
                radiusDrawable.SetCornerRadius(Frame.Context.ToPixels(_sheet.CornerRadius));
                Frame.Background = radiusDrawable;
                /*
                if (Frame.Background is not SheetRadiusDrawable radiusDrawable)
                {
                    radiusDrawable = new SheetRadiusDrawable();
                    Frame.Background = radiusDrawable;
                }
                radiusDrawable.SetCornerRadius(Frame.Context.ToPixels(_sheet.CornerRadius));
                */
            ...
            if (paint is not null)
            {
                ...
                if (Frame.Background is GradientDrawable drawable)
                {
        ...
        else if (Frame.Background is ColorDrawable colorDrawable)
        {
    ...
    void EnsureWindowContainer(Context context)
    {
        EnsureStayOnFrontView(context);
        if (_windowContainer is null)
        {
            var container = (FrameLayout)AView.Inflate(context, Resource.Layout.bottom_sheet_design, null);
...
#pragma warning restore CS0618
    ...
    public void Show(bool animated)
    {
        ...
        if (_sheet.HasHandle)
        {
            //_handle = new BottomSheetDragHandleView(context, null, Microsoft.Maui.Controls.Resource.Style.Widget_Material3_BottomSheet_DragHandle);
